...
import { IPromptState } from './prompt-interfaces';
const moment: any = moment_;

export const promptFormatter = (fieldName: string, state: IPromptState) => {
  let formatted: any;
  ...
  switch (field) {
    ...
    case 'dateDisplay':
      if (state && state.startTime)
        formatted = moment(state['startTime']).format('dddd DD MMMM');
      break;
    case 'dateDisplayShort':
      if (state && state.startTime)
        formatted = moment(state['startTime']).format('DD-MM-YY');
      break;
    default:
